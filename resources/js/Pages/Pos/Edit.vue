                                ...
                                <div class="col-sm-6 mb-3" v-if="company_id">
                                    <div class="user-search form-group position-relative">
                                        <v-select
                                            ...
                                            label="car_number"
                                            v-model="car_number"
                                            :reduce="(option) => option.car_number"
                                        />
                                        <span class="invalid-feedback d-block"></span>
                                    </div>
                                </div>
...
export default {
    ...
    methods: {
        singleOrder: function (isPrint = false, isEdit = false) {
            ApiService.POST(ApiRoutes.SaleSingle, {id: this.saleId}, res => {
                if (parseInt(res.status) === 200) {
                    ...
                    if (isEdit) {
                        this.company_id = res.data.payment_category_id;
                        ...
                        setTimeout(() => {
                            ...
                            this.driver_sale.driver_id = res.data.driver_id;
                            $('.date').flatpickr({
                                altInput: true,
                                altFormat: "d/m/Y",
                                dateFormat: "Y-m-d",
        ...
        order: function (type) {
            ...
            ApiService.POST(ApiRoutes.SaleEdit, param, res => {
                if (type == 'cash') {
                    this.loading = false
                } else if (type == 'company') {
